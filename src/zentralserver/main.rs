...
fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn std::error::Error>>//,
{
    ...
    let sender_id_str = from_utf8(&buff[0..8]).unwrap();
    let sender_id: i32 = sender_id_str.parse().unwrap();
    let second_inf_str = from_utf8(&buff[8..16]).unwrap();
    let second_inf: i32 = second_inf_str.parse().unwrap();
    let command_str = from_utf8(&buff[16..20]).unwrap();
    let command: i32 = command_str.parse().unwrap();
    let data_strem = from_utf8(&buff[20..]).unwrap();
